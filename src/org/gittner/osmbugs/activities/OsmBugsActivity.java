...
public class OsmBugsActivity extends Activity implements LocationListener, OnItemGestureListener<Bug>, OnTouchListener{
	...
	/* Public static because the Bug Editor gets its Bug by the position in this List */
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		/* Try to get a Coarse Location as a starting Point */
		Location fastLoc = locMgr_.getLastKnownLocation(LocationManager.NETWORK_PROVIDER);
		if(fastLoc != null)
			onLocationChanged(fastLoc);
	}
	...
	private void createBug(int platform, String text) {
		...
		if(platform == OPENSTREETBUGS){
			bug = new OpenstreetbugsBug(newBugLocation_.getLatitudeE6() / 1000000.0 , newBugLocation_.getLongitudeE6() / 1000000.0 , "", new ArrayList<Comment>(), -1, Bug.STATE.OPEN);
			bug.addComment(text);
		}
		else if(platform == OPENSTREETMAPNOTES){
			bug = new OpenstreetmapNote(newBugLocation_.getLatitudeE6() / 1000000.0 , newBugLocation_.getLongitudeE6() / 1000000.0 , "", new ArrayList<Comment>(), -1, Bug.STATE.OPEN);
			bug.addComment(text);
		}
		else
			return;
