...
public class OsmBugsActivity extends Activity implements LocationListener, OnItemGestureListener<Bug>, OnTouchListener {
	...
	private boolean shouldCenterGPS = false;
	...
	public void onLocationChanged(Location location) {
		...
		/* Center Map on GPS Position if button pressed */
		if (shouldCenterGPS) {
			shouldCenterGPS = false;
			mapView_.getController().setCenter(new GeoPoint(location));
		}
	...
	public boolean onTouch(View v, MotionEvent event) {
		if (v.getId() == R.id.mapview) {
			if (event.getAction() == MotionEvent.ACTION_DOWN) {
				...
				float diff = (float) Math.sqrt(Math.pow(startPoint.x - event.getX(), 2) + Math.pow(startPoint.y - event.getY(), 2));
				if (diff > 50)
					mapViewDownTimeStart_ = System.currentTimeMillis();
